...
export function TrainingForm({ onSubmit, onCheckExisting, onUpdate, loading, lastRecord, weeklyAverage, daysWithData = 0, consecutiveDays = 0 }: TrainingFormProps) {
  ...
  const [pendingData, setPendingData] = useState<{ rpe: number; duration_min: number; date: string } | null>(null);
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('duplicate key value violates unique constraint')) {
          setError(`${new Date(selectedDate).toLocaleDateString('ja-JP')}の記録は既に存在します。既存の記録を編集してください。`);
        } else {
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {successFeedback && (
        <div className={`rounded-lg border-2 p-4 animate-in slide-in-from-top-2 duration-300 ${
      ...
      {lastRecord && (
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-sm">
          ...
          <p className="text-blue-700 dark:text-blue-300">
            RPE {lastRecord.rpe} × {lastRecord.duration_min}分 = 負荷 {lastRecord.rpe * lastRecord.duration_min}
          </p>
        </div>
      )}
      ...
      {weeklyAverage && (
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-sm">
          ...
          <p className="text-gray-700 dark:text-gray-300">
            RPE {weeklyAverage.rpe.toFixed(1)} × {weeklyAverage.duration.toFixed(0)}分 = 負荷 {weeklyAverage.load.toFixed(0)}
          </p>
        </div>
      )}
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
          {selectedDate === new Date().toISOString().split('T')[0] ? '今日の練習記録' : '過去の練習記録'}
        </p>
      </div>
        ...
        <div className="space-y-2">
          <input
            ...
            style={{
              ...
              background: `linear-gradient(to right, #3b82f6 0%, #3b82f6 ${(rpe / 10) * 100}%, #9ca3af ${(rpe / 10) * 100}%, #9ca3af 100%)`
            }}
          />
      ...
      <div>
        ...
        <input
          ...
          style={{ fontSize: '16px' }}
        />
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">1分〜480分（8時間）まで入力可能</p>
      </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 transition-colors">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-lg font-semibold text-gray-900 dark:text-white">{rpe * duration}</span>
        </div>
        <p className="text-xs text-gray-500 dark:text-gray-400">負荷 = RPE × 練習時間</p>
      </div>
