...
import { User, TrainingRecord, supabase } from '../lib/supabase';
...
interface AthleteWithACWR extends User {
  ...
  recentTrainingDays?: number; // 最近の練習日数を追加
}

export const AthleteList = React.memo(({ athletes, onAthleteSelect }: AthleteListProps) => {
  ...
  const fetchAthletesACWR = useCallback(async () => {
    try {
      ...
      for (const athlete of athletes) {
        // Fetch training records for each athlete
        ...
        // 最近28日間の実際の練習日数を計算（ACWRデータがある日のみ）
        ...
        const recentTrainingDays = acwrData.filter(data => {
          ...
          return dataDate >= twentyEightDaysAgo && dataDate <= today && data.acwr > 0;
        }).length;
  ...
  if (athletes.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600">このチームに登録されている選手がいません。</p>
      </div>
    );
  }
              ...
              <div className="bg-white rounded-lg p-2">
                ...
                {athlete.latestACWR ? (
                  <div
                    ...
                    style={{ color: getRiskColor(athlete.riskLevel || 'good') }}
                  >
                    {athlete.latestACWR}
                  </div>
                ) : (
