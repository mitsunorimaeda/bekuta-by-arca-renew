...
export function UserManagement({ teams, restrictToOrganizationId }: UserManagementProps) {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      // ✅ 念のため：ALL が来ても全件扱いにする（事故防止）
      const orgId =
        !restrictToOrganizationId || restrictToOrganizationId === 'ALL'
          ? undefined
          : restrictToOrganizationId;

      if (orgId) {
        // ✅ 組織で絞る：organization_members -> users を JOIN で取得
      ...
      setError('ユーザーの取得に失敗しました: ' + (err?.message || 'unknown error'));
    } finally {
      setLoading(false);
    }
  }, [restrictToOrganizationId]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-purple-600">{users.filter((u: any) => u.role === 'global_admin').length}</div>
          <div className="text-sm text-purple-700">管理者</div>
        </div>
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-green-600">{users.filter((u: any) => u.role === 'staff').length}</div>
          <div className="text-sm text-green-700">スタッフ</div>
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-blue-600">{users.filter((u: any) => u.role === 'athlete').length}</div>
