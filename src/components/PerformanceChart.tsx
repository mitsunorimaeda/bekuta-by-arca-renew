...
interface PerformanceChartProps {
  ...
  // ★追加：表示単位（primary_valueなら "kg" / relative_1rmなら "×BW" など）
  unitLabel?: string;
...
export const PerformanceChart = React.memo(function PerformanceChart({
  ...
  // 単位（props優先、なければ testType.unit、相対1RMなら×BW）
  ...
  const latestAthleteValue = useMemo(() => {
    // athletePointsは日付昇順なので最後が最新（value nullの可能性はあるので逆走査）
      ...
      <ResponsiveContainer width="100%" height={400}>
        <LineChart data={chartData as any[]} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <Line
            type="monotone"
            dataKey="value"
            name={metricKey === 'relative_1rm' ? '個人（相対）' : (testType?.display_name || '個人')}
            stroke="#3B82F6"
            strokeWidth={3}
          ...
          {teamMonthly?.length > 0 && (
            <Line
              type="monotone"
              dataKey="team_avg"
              name={metricKey === 'relative_1rm' ? 'チーム月次平均（相対/最新）' : 'チーム月次平均（最新）'}
              stroke="#6366F1"
              strokeWidth={2}
              dot={{ r: 3 }}
              activeDot={{ r: 5 }}
