/*
  ...
  1. 新しいテーブル
    - `weight_records`
      ...
      - `user_id` (uuid, foreign key) - ユーザーID（auth.usersテーブルへの外部キー）
      - `date` (date) - 体重測定日
...
DROP POLICY IF EXISTS "Users can view own weight records" ON weight_records;
...
DROP POLICY IF EXISTS "Users can update own weight records" ON weight_records;
...
DROP POLICY IF EXISTS "Users can delete own weight records" ON weight_records;
