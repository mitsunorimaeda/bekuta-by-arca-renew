...
-- Note: To schedule automatic cleanup with pg_cron (requires extension):
-- 1. Enable pg_cron extension (requires superuser or RDS admin):
--    CREATE EXTENSION IF NOT EXISTS pg_cron;
--
-- 2. Schedule daily cleanup at 2 AM:
--    SELECT cron.schedule('cleanup-old-messages', '0 2 * * *', 'SELECT cleanup_messaging_data()');
--
-- For Supabase free tier, manual cleanup is recommended:
-- Run this query periodically (e.g., weekly via admin panel):
-- SELECT cleanup_messaging_data();
